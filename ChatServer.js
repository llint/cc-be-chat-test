...
export function buildProfanityWordsTrie(words) {
    var root = {}; // {char: next}
...
export function detectAndReplaceProfanityWords(word) {
        ...
        wss.on('connection', function connection(ws) {
            ...
            ws.on('message', function incoming(message) {
                ...
                if (splitted[0] == "/popular") {
                    // this command causes trimming the expired entries as well
                    this.trimExpiredPopularWords(Date.now());
                    ...
                    // now process the words for popularity
                    this.updatePopularity(splitted);
                }
            });
        });
    }

    updatePopularity(inputWords) {
        var now = Date.now();
        ...
        this.trimExpiredPopularWords(now);
    }
